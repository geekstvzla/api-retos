CREATE 
VIEW `vw_event_edition_optional_item` AS
    SELECT 
        `eeoi`.`event_edition_id` AS `event_edition_id`,
    ...
    FROM
        (((`event_edition_optional_item` `eeoi`
        JOIN `currencies` `c` ON ((`c`.`currency_id` = `eeoi`.`currency_id`)))
        JOIN `currencies_lang` `cl` ON ((`cl`.`currency_id` = `c`.`currency_id`)))
