...
const sqlPath = __dirname + '/sp_user_enroll.sql';
let rawSql = fs.readFileSync(sqlPath, 'utf8');

const dropStmt = 'DROP PROCEDURE IF EXISTS `sp_user_enroll`';
...
db.connect((err) => {
    ...
    db.query(dropStmt, (dropErr) => {
        if (dropErr) {
            console.error('Drop error:', dropErr.message);
            process.exit(1);
        }
        db.query(createStmt, (createErr) => {
            ...
            console.log('SUCCESS: Stored procedure sp_user_enroll updated in database.');
            db.end();
        });
    });
});
